...
public interface PositionRepository extends CustomizeRepository<Statistics, LocalDateTime> {
    ...
    @Query(value = "SELECT\n" +
            ...
            " (sum(to_dispatch)*100*0.2+(sum(in_time_dispatch)/sum(need_dispatch))*100*0.4+(sum(need_dispatch)-ifnull(sum(rework),0))/sum(in_time_dispatch)*100*0.4) as aggregativeIndicator,\n" +
            "#评价等级\n" +
            "(CASE \n" +
