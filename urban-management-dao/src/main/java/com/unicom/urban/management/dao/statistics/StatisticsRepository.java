...
public interface StatisticsRepository extends CustomizeRepository<Statistics, String> {
     ...
     * @return 数据
     */
    @Query(value = "SELECT SUM(inst) as totalInst,sum(`close`) as totalClose,sum(`dispose`) as totalDispose,g.id as gridId  from `event` ev,grid g,statistics st where g.id=ev.grid_id and ev.id=st.event_id and ev.create_time between ?1 and ?2 GROUP BY g.id order by totalInst", nativeQuery = true)
     ...
     * @return list
     */
    @Query(value = "SELECT count(s.id) as totalInst, et.name as name, et.id as id FROM Statistics s LEFT JOIN s.event e LEFT JOIN e.eventType et WHERE s.inst = 1 and e.createTime between ?1 and ?2 GROUP BY e.eventType order by totalInst ")
